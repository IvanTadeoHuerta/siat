...
require(['routing'], function (routing) {
    ...
    $.widget("custom.estructuraGeneral", {
        ...
        _contruirEstructura: function _contruirEstructura() {
            return '<div id="wrapper">' + this._contenedorMenus() + this._panelContenidoFormularios() + '</div';
        },

        _contenedorMenus: function _contenedorMenus() {
            return '<!-- Navigation -->' + '<nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">' +
            //Espacio para incluir los diferentes menus de la pantalla generica
            this._cabecera() + this._menuSuperior() + this._menuLateral() +
            //Fin de espacio de los menus
            '</nav>';
        },

        _cabecera: function _cabecera() {
            return '<div class="navbar-header">' + '<button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">' + '<span class="sr-only">toggle navigation</span>' + '<span class="icon-bar"></span>' + '<span class="icon-bar"></span>' + '<span class="icon-bar"></span>' + '</button>' + '<a class="navbar-brand" href="#">' + this.options.tituloHeader + '</a>' + '</div>';
        },

        _menuSuperior: function _menuSuperior() {
            return '<ul class="nav navbar-top-links navbar-right">' + '<!-- /.dropdown -->' + '<li class="dropdown">' + '<a class="dropdown-toggle" data-toggle="dropdown" href="#">' + '<i class="fa fa-bell fa-fw"></i> <i class="fa fa-caret-down"></i>' + '</a>' + '<ul class="dropdown-menu dropdown-alerts">' + '<li>' + '<a href="#">' + '<div>' + '<i class="fa fa-envelope fa-fw"></i> Message Sent' + '<span class="pull-right text-muted small">4 minutes ago</span>' + '</div>' + '</a>' + '</li>' + '<li class="divider"></li>' + '<li>' + '<a class="text-center" href="#">' + '<strong>Ver todas</strong>' + '<i class="fa fa-angle-right"></i>' + '</a>' + '</li>' + '</ul>' + '<!-- /.dropdown-alerts -->' + '</li>' + '<!-- /.dropdown -->' + '<li class="dropdown">' + '<a class="dropdown-toggle" data-toggle="dropdown" href="#">' + '<i class="fa fa-user fa-fw"></i> <i class="fa fa-caret-down"></i>' + '</a>' + '<ul class="dropdown-menu dropdown-user">' + '<li><a href="#"><i class="fa fa-user fa-fw"></i> Mi perfil</a>' + '</li>' + '<li><a href="#"><i class="fa fa-gear fa-fw"></i> Configuraciones</a>' + '</li>' + '<li class="divider"></li>' + '<li class="logoutUser"><a href="#"><i class="fa fa-sign-out fa-fw"></i> Cerrar sesión</a>' + '</li>' + '</ul>' + '<!-- /.dropdown-user -->' + '</li>' + '<!-- /.dropdown -->' + '</ul>' + '<!-- /.navbar-top-links -->';
        },
        _arrayObjectIndexOf: function _arrayObjectIndexOf(myArray, searchTerm) {
            var bandera = false;
        ...
        _menuLateral: function _menuLateral() {
            var menu = '';
            for (var indice in this.options.datosUsuario) {
                ...
                for (var elemento in this.options.datosUsuario[indice]) {
                    if (elemento == 'nombreProceso') {
                        tmpMenu += '<li>' + '<a href="#" name="menuPrincipal"  modulo=modulo' + this.options.datosUsuario[indice].idProceso + 'class="' + clase + '"><i class="fa fa-dashboard fa-fw"></i>' + this.options.datosUsuario[indice].nombreProceso;
                    }
                    ...
                    if (elemento == 'subMenu') {
                        ...
                        for (var x in this.options.datosUsuario[indice][elemento]) {
                            for (var y in this.options.datosUsuario[indice][elemento][x]) {
                                if (y == 'nombreProceso') {
                                    tmpSubMenuString += '<li><a href="#" modulo=modulo' + this.options.datosUsuario[indice][elemento][x].idProceso + ' name="subMenu1" class="optionMenuItem">' + this.options.datosUsuario[indice][elemento][x].nombreProceso + '</a></li>';
                                }
                            }
                        }
                        tmpSubMenuString += '</ul>';
            ...
            '<li>'+
                ...
                '<!-- /.nav-second-level -->'+
            '</li>'+*/
            '</ul>' + '</div>' + '<!-- /.sidebar-collapse -->' + '</div>';
        },
        _panelContenidoFormularios: function _panelContenidoFormularios() {

            return '<div id="page-wrapper">' + '<div class="row">' + '<div class="col-lg-12">' + '<h5 class="page-header">' + this.options.tituloFormulario + '</h5>' + '</div>' + '<!-- /.col-lg-12 -->' + '</div>' + '</div>';
        },

        _eventos: function _eventos() {
            $('#side-menu').metisMenu();
